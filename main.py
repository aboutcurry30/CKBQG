...
sys.path.append(r'/home/ylwang/newspace2/CJY/KBQG-main')
import torch
...
# from fastNLP import BucketSamplers
...
class Bert2tf(nn.Module):
    ...
    #     if words.size(1) > self.encoder.config.max_position_embeddings:
    #         words = words[:, :self.encoder.config.max_position_embeddings]
    # # Truncate target if it exceeds max_seq_length - 1
    #     # if target.size(1) > self.encoder.config.max_position_embeddings - 1:
    #     #     target = target[:, :self.encoder.config.max_position_embeddings - 1]
...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    # parser.add_argument('--train_file', type=str, default="/home/ylwang/newspace2/CJY/KBQG-main/KBQG data/train.json")
    # parser.add_argument('--dev_file', type=str, default="/home/ylwang/newspace2/CJY/KBQG-main/KBQG data/dev.json")
    # parser.add_argument('--test_file', type=str, default="/home/ylwang/newspace2/CJY/KBQG-main/KBQG data/test.json")
    parser.add_argument('--train_file', type=str, default="/home/ylwang/newspace2/CJY/KBQG-main/duibidata/train.json")
    parser.add_argument('--dev_file', type=str, default="/home/ylwang/newspace2/CJY/KBQG-main/duibidata/dev.json")
    parser.add_argument('--test_file', type=str,
                        default="/home/ylwang/newspace2/CJY/KBQG-main/knowledge graph/test.json")
    ...
    # training
    parser.add_argument('--mode', type=str, default="test", help="training or testing")
...
def train(config: Config, train_dataloader: DataLoader, num_epochs: int, early_stop: int,
    ...
    model.to(dev)
    # os.environment['CUDA_VISIBLE_DEVICES'] = '0,1,2'
    device_ids = [0, 1, 2, 3]
    if config.gpus:
...
def test(config: Config, valid_dataloader: DataLoader, model: nn.Module, dev: torch.device,
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
        for index, batch in tqdm(enumerate(valid_dataloader), desc="--validation", total=len(valid_dataloader)):
            ...
            target = [tokenizer.decode(t[1:-1], skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for
                      t in
                      batch.question_ids]
    ...
    print(nltk.translate.bleu_score.corpus_bleu(golds, preds, weights=(0.25, 0.25, 0.25, 0.25)))
    print(m_score / num)
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained("/home/ylwang/newspace2/CJY/KBQG-main/bert-base-uncased")
